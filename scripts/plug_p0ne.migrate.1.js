...
var migrate, out$ = typeof exports != 'undefined' && exports || this;
...
(migrate = function(){
  var vArr, migrated, v, dest, i$, ref$, len$, key, k, err, res$, m, renameMap, oldClass, newClass, perCommunityModules, that, noMutedSongs, avatarID, vanillaAvatarID, remaining, itemSet, data, x, yet$;
  migrated = function(v){
  ...
  switch (false) {
  case compareVersions(v, dest = '1.2.0'):
    ...
    migrated(dest);
    // fallthrough
  case compareVersions(v, dest = '1.3.3'):
    ...
    migrated(dest);
    // fallthrough
  case compareVersions(v, dest = '1.3.9'):
    ...
    for (i$ = 0, len$ = (ref$ = ['requireIDs', 'automute', 'moduleSettings']).length; i$ < len$; ++i$) {
      k = ref$[i$];
    ...
    migrated(dest);
    // fallthrough
  case compareVersions(v, dest = '1.5.0'):
    ...
    migrated(dest);
    // fallthrough
  case compareVersions(v, dest = '1.6.1'):
    console.info("[p0ne migrate] renamed a few modules and CSS classes");
    editLocalStorage('moduleSettings', function(moduleSettings){
      var renameMap, oldModuleName, newModuleName, i$, ref$, len$, module;
      renameMap = {
        _API_: 'InternalAPI',
        p0neChatInput: 'betterChatInput'
      };
      for (oldModuleName in renameMap) {
        newModuleName = renameMap[oldModuleName];
      ...
      for (i$ = 0, len$ = (ref$ = ['fixGhosting', 'fixOthersGhosting', 'fixNoPlaylistCycle']).length; i$ < len$; ++i$) {
        module = ref$[i$];
  ...
  case compareVersions(v, dest = '1.6.5'):
    ...
    migrated(dest);
    // fallthrough
  case compareVersions(v, dest = '1.7.0'):
    ...
    if (that = localStorage.p0ne_moduleSettings) {
      localStorage.p0ne_disabledModules = that;
    }
    delete localStorage.moduleSettings;
    noMutedSongs = true;
    editLocalStorage('automute', function(songlist){
      var res, k;
    ...
    avatarID = localStorage.avatarID, vanillaAvatarID = localStorage.vanillaAvatarID;
    if (avatarID || vanillaAvatarID) {
      localStorage.p0ne_customAvatars = compress(
      JSON.stringify(
      {
        avatarID: avatarID,
        vanillaAvatarID: vanillaAvatarID
      }));
    }
    remaining = 1;
    itemSet = function(){
...
function editLocalStorage(moduleName, cb){
  var data, err;
  try {
    if (!(data = localStorage["p0ne_" + moduleName])) {
      return false;
    }
    data = JSON.parse(
    decompress(
    data));
    cb(data);
    localStorage["p0ne_" + moduleName] = compress(
    JSON.stringify(
    data));
    return true;
  } catch (e$) {
...
function editLocalforage(moduleName, cb){
  return localforage.getItem("p0ne_" + moduleName, function(err, data){
    if (err) {
      return console.error("failed to read '" + moduleName + "' using localforage");
    }
    try {
      if (!data) {
        return false;
