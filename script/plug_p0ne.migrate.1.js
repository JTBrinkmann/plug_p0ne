 ...
 */
(function(){
  var vArr, migrated, v, i$, ref$, len$, key, k, err, res$, ref1$, m, renameMap, oldClass, newClass, perCommunityModules;
  migrated = function(v){
  ...
  case compareVersions(v, '1.5.0'):
    ...
    migrated('1.5.0');
    // fallthrough
  case compareVersions(v, '1.6.1'):
    console.info("[p0ne migrate] renamed a few modules and CSS classes");
    editSettings('moduleSettings', function(moduleSettings){
...
function editSettings(moduleName, cb){
  var settings, err;
  try {
    if (!(settings = localStorage["p0ne_" + moduleName])) {
      return false;
    }
    settings = JSON.parse(
    decompress(
